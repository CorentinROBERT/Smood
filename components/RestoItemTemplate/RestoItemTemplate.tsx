...
function _RestoItemTemplate({ resto, isLarge = false }: { resto: Restaurant; isLarge?: boolean }) {
    ...
    <View style={isLarge ? { ...s.largeContainer, width: width - 20 } : s.container}>
      ...
      <View style={s.infos}>
        ...
        <View style={isLarge ? s.tagsLarge : s.tags}>
          {resto.storeTags.reduce<React.ReactNode[]>((acc, storeTag, index) => {
            acc.push(
              <Text key={storeTag.id} numberOfLines={3} ellipsizeMode="clip" style={s.tagName}>
                {storeTag.name}
              </Text>
            );
            ...
            if (index < resto.storeTags.length - 1) {
              acc.push(
                <Text key={`${index}`} style={s.separator}>
                  {' '}
                  ·{' '}
                </Text>
              );
        ...
        <View style={s.timeFee}>
          ...
          <Text>
            {' '}
            {resto.estimatedArrivalTime} min - Fees : {resto.deliveryFee.toFixed(2)} CHF
          </Text>
        </View>
