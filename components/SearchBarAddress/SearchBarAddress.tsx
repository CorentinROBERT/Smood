import { TextInput, TouchableOpacity, View, Text, ViewStyle } from 'react-native';
...
export function SearchBarAddress({ style }: { style: ViewStyle }) {
  const [isSearchResult, setIsSearchResult] = useState(false);
  const [proposals, setProposals] = useState<AddressProposal[]>([]);
  const [isProposalVisible, setIsProposalVisible] = useState(false);
  const [searchText, setSearchText] = useState('');
  const { setAddress } = useContext(SmoodContext);

  async function getAddressFromName(addressName: string) {
